...
def main():
    ...
    ids_with_n_subjects_dir = experiment_dir / 'bootstrap_analysis' / ('{:02d}'.format(n_gender_pairs))
    ...
    for i_bootstrap in range(n_bootstrap):
        ids_filename = 'homogeneous_bootstrap_{:04d}_n_{:02d}.csv'.format(i_bootstrap, n_gender_pairs)
        id_path = ids_with_n_subjects_dir / 'ids' / ids_filename

        dataset_filename = 'homogeneous_bootstrap_{:04d}_n_{:02d}.h5'.format(i_bootstrap, n_gender_pairs)
