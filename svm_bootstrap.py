...
def main():
    ...
    # ----------------------------------------------------------------------------------------
    # Add independent dataset biobank scanner 2
    independent_dataset_path = experiment_dir / 'biobank2.h5'
    independent_dataset = pd.read_hdf(independent_dataset_path, key='table')

    regions = independent_dataset[COLUMNS_NAME].values
    tiv = independent_dataset.EstimatedTotalIntraCranialVol.values[:, np.newaxis]

    x_independent = np.true_divide(regions, tiv)
    y_independent = independent_dataset['Age'].values
    ...
    for i_n_subject_pairs in range(1, 51):
        ...
        for i_bootstrap in range(n_bootstrap):
            ...
            training_dataset_filename = 'homogeneous_bootstrap_{:04d}_n_{:02d}_train.h5'.format(i_bootstrap, i_n_subject_pairs)
            test_dataset_filename = 'homogeneous_bootstrap_{:04d}_n_{:02d}_test.h5'.format(i_bootstrap, i_n_subject_pairs)
            ...
            x_independent_norm = scaler.transform(x_independent)

            # Systematic search for best hyperparameters
            # TODO: Change to Pedro's RVR
            ...
            r2_score = svm.score(x_test, y_test)
            ...
            independent_predictions = svm.predict(x_independent_norm)

            absolute_error = mean_absolute_error(y_independent, independent_predictions)
            root_squared_error = sqrt(mean_squared_error(y_independent, independent_predictions))
            r2_score = svm.score(x_independent_norm, y_independent)
            age_error_corr, _ = stats.spearmanr(np.abs(y_independent - independent_predictions), y_independent)
            ...
            # Save arrays with permutation coefs and scores as np files
            filepath_scores = scores_dir / ('boot_scores_{:04d}_independent.npy'.format(i_bootstrap))
            np.save(str(filepath_scores), mean_scores)
