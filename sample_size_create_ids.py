...
def main():
    ...
    id_path = PROJECT_ROOT / 'outputs' / experiment_name / 'homogenized_ids.csv'
    # ----------------------------------------------------------------------------------------
            ...
            for age in range(age_min, (age_max + 1)):
                ...
                for gender in range(2):
                    ...
                    dataset_bootstrap_test = pd.concat(
                        [dataset_bootstrap_test, pd.DataFrame(random_sample_test['Image_ID'])])

            # Export dataset_bootstrap_train as csv
            ids_filename = 'sample_size_{:04d}_n_{:02d}_train.csv'.format(i_bootstrap, i_n_subject_pairs)
            dataset_bootstrap_train.to_csv(ids_dir / ids_filename, index=False)

            ids_filename = 'sample_size_{:04d}_n_{:02d}_test.csv'.format(i_bootstrap, i_n_subject_pairs)
