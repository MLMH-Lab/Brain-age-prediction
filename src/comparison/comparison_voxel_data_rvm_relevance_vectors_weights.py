#!/usr/bin/env python3
"""Script to calculate the relevance vectors of the Relevance Vector Machine
(RVM) approach for voxel-level data.
...
def main(experiment_name, input_path_str, input_ids_file, input_data_type, mask_filename):
    ...
    for i, subject_id in enumerate(tqdm(ids_df['image_id'])):
        # Check if subject is support vector in any model before loading the image
